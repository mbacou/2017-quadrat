...
##########################################
# Load grids year by year over 2005-2014 10-year period and return max grid
...
setnames(stats, ".id", "year")
stats[, date := as.Date(paste0(year, "-", rep(1:12, each=3),"-", rep(c(1,11,21), 12)))]

# Plot time-series
xyplot(`Max.`+`Min.`+Median~ date, stats,
  ylab="mm", xlab="Long-Term NDVI 2005-2015 (West Africa)",
  panel=function(x, y, ...) {
    panel.grid(h=-1, v=0, lty=2)
    panel.xyplot(x, y, ..., type="l", col=viridis(3), lwd=1)
    panel.abline(v=stats[, mean(`Max.`)], col="grey90")
  },
  key=list(text=list(c("Max.", "Min.", "Median")),
    col=viridis(3), lty=c(1,1,1), lines=T, columns=3),
)
...
m <- tm_shape(crop(out, gha)) + tm_raster(names(out), pal=pal.ndvi(255),
  ...
  legend.hist=T) + tm_layout(legend.outside.size=0.4) + p
