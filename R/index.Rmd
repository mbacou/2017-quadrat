...
tmp <- extract(r, g2, fun=mean, na.rm=T)
summary(tmp[,1])
g2$glues <- tmp[,1]
...
rm(tmp, tmp.mean, tmp1, m, out, i, x, r)
...
```{r, fig.cap="NDVI Long-Term Monthly Mean and Spatial Variation (2005-2015, Ghana)", fig.height=3, fig.width=5}
...
ggplot(stats.month, aes(date, mean, group=1)) +
  ...
  xlab("") + ylab("") + theme_bw(base_size=10) + 
