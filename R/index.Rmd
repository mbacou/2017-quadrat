...
load("./tmp/2017-quadrat.RData")
rasterOptions(tmpdir="/home/data/tmp")
...
#####################################################################################
# L14IGB3 - Cropland/natural vegetation mosaic based on the MOD12Q1 product
# http://worldgrids.org/doku.php/wiki:l14igb3 (2011, percent, 1km)
tmp <- raster("~/Projects/hc-data/WorldGrids/l14igb3.tif")
res(tmp)
# [1] 0.008333333 0.008333333 => 1km
crs(tmp)
# +proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0
...
# Append these new grids into `fews`
fews <- rbind(fews, ndvi_west, fill=T)
rm(ndvi_west)

##########################################
# Load grids year by year over 2005-2015 10-year period and return max value
fun <- function(x) {
  x[x > 200] <- NA
  x <- max(x[1:36], na.rm=T)
  x <- x - 100 # Keep values as INT2S *100 to save disk space
  return(x)
}

# Init list of yearly statistics
stats <- vector("list", 10)
names(stats) <- as.character(2005:2014)

for (i in (2005:2014)[2]) {
  tmp <- stack(as.list(fews[varCode=="ndvi_west" & year(date)==i, raster]))
  showTmpFiles()
  removeTmpFiles(h=0)
  # Save grid statistics for plotting
  stats[[as.character(i)]] <- summary(tmp)
  cat(i, "\n")
  # Return yearly max value
  out <- calc(tmp, fun, filename=paste0(dir, "ndvi_west/wa", i, "_max.tif"),
    datatype="INT2S", overwrite=T)
}

spplot(crop(out, gha), col.regions=pal.ndvi(255), zlim=c(0,90))
...
m <- tm_shape(tmp.mean) + tm_raster(names(tmp.mean), pal=pal.ndvi(255),
  title="10-Year Max Yearly NDVI\n2005/01-2015/12\n(index)",
  breaks=seq(-1,1.05,0.05), labels=na2null(seq(-1,1,0.05)*c(1, NA))) + p
...
m <- tm_shape(crop(tmp, gha)) + tm_raster(names(tmp), pal=pal.ndvi(255),
  title="GLUES\nCrop Suitability\n(all crops)\n(percent)",
  breaks=seq(0,105,5), labels=na2null(seq(0,100,5)*c(1, NA))) + p
...
# 2) Mask out protected areas (rasterize WDPA shapes first)
...
include_graphics(c(
  "../docs/out/esa-lc.png",
  "../docs/out/usgs-lc.png",
  "../docs/out/fews-chirps.png",
  "../docs/out/fews-rfe.png",
  "../docs/out/modis-cropland.png",
  "../docs/out/glues-suit.png"
  ))

```

ESA LULC grids are combined with IUCN WDPA protected areas to create areas of low/high crop suitability. MODIS C6 dekadal NDVI is summarized over a 10-year period (2005-2015).

```{r, eval=FALSE, out.width="380px"}

include_graphics(c(
  "../docs/out/fews-ndvi.png",
  "../docs/out/cropland-mask.png"
  ))
