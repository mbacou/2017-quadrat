...
2. Mask out forest and urban areas based on ESA LULC map at 300m resolution. The exact classes that are considered not (low) suitable are:  

```{r}

kable(lc.lbl[mask==T, .(Code=code, Label=class)], caption="LULC Categories not Suitable for Farming (Source: ESA/CCI Land Cover Classification, 2015)")
...
5. Overlay the suitability raster with IFPRI/HarvestChoice Travel Time to **100K Market** reclassified into zones of low/high market access using a **cutoff at 4hrs** (2010 version resampled from 10km to 1km)  

```{r, fig.cap="Protected Areas not Suited for Agriculture (source: WDPA 2017)", out.width="380px"}
...
```{r, fig.cap="Agricultural Suitability (after masking out unsuitable land use/cover and protected areas)", out.width="380px"}
...
```{r, fig.cap="Quadrant Classification, Ghana and Nigeria", out.width="380px"}
